...
class FavouriteAdapter constructor(context: Context, val callData: CallInterface) :
    ...
    override fun onViewHolderBind(
        ...
        if (!data.profile.isNullOrBlank()) {
            val imageArray = (context as BaseActivity).convertStringToByteArray(data.profile)
            val bitmap = BitmapFactory.decodeByteArray(imageArray, 0, imageArray.size)
            Glide.with(context).load(bitmap)
                .into(binding.circleImageView)
        } else {
            Glide.with(context).load(R.drawable.avtar_profile)
                .into(binding.circleImageView)
        }
    ...
    override fun onClickItemListner(
        binding: ItemFavouriteBinding,
        data: ContactList,
        position: Int
    ) {
